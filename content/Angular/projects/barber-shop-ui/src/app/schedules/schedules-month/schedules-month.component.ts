...
@Component({
  ...
  providers: [
    { provide: SERVICE_TOKEN.HTTP.SCHEDULE, useClass: SchedulesService },
...
export class SchedulesMonthComponent implements OnInit, OnDestroy {
  ...
  constructor(
    @Inject(SERVICE_TOKEN.HTTP.SCHEDULE) private readonly httpService: IScheduleService,
    @Inject(SERVICE_TOKEN.HTTP.CLIENT) private readonly clientHttpService: IClientService,
    @Inject(SERVICE_TOKEN.SNACKBAR) private readonly snackbarManage: ISnackbarManagerService
  ) { }
